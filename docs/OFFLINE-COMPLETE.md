...
整合分支 `offline/complete-portable` 基于已合并 PR #2 的 develop，保留加密实现并选择性纳入 PR #1 的资源改进。不要再直接把两个旧分支强行覆盖合并。以整合 PR 最新源码的质量检查和两个原生包/启动检查为准；失败时不管理员绕过、不强推 develop。

发布使用 `Publish validated offline portable release` 工作流。全部检查通过后，在 PR 描述中记录唯一的 `<!-- offline-validation-run: 实际成功的运行编号 -->`，将 PR 标记 Ready for review，采用 **Create a merge commit** 正常合并到本仓库 develop，保留已验证的 head 提交。不要在验证后新增修改再沿用旧运行编号。

发布作业核对 PR 已合并、构建源码为该 PR head、合并提交保留此 head 为父提交，且两者完整 Git tree 相同；再核对指定 Actions 运行成功、两个平台的实际运行证据、完整目录、归档大小和 SHA-256。使用测试过的原始附件，不在发布阶段重编译或更换内容。标签指向真实构建源码，`release-provenance.json` 同时记录合并 SHA、源 SHA、相同 Git tree 和验证运行。附件先上传为 Draft，GitHub 服务端摘要及大小全部匹配后才公开候选发行版。

正常 develop 构建仍会进行独立复核，但不自动发布旧加密子集，也不把重编译出的另一组字节冒充已经测试的附件。若合并有冲突或源码树变化，发布必须失败并要求验证新源码；不能绕过。源码合并和高安全现场验收是两项不同工作。
