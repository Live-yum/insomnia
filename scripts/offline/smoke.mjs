...
import { verifySelectedEdition } from './select-smoke-edition.mjs';
...
try {
  ...
  assert.equal(await page.evaluate(async () => {
    try { const response = await fetch('https://api.insomnia.rest/v1/user'); return response.status === 403; } catch { return true; }
  }), true);
  ...
  await verifySelectedEdition(app, page);
  console.log('Packaged offline smoke passed: actual edition, no login, persisted local project, authenticated crypto bridge, vendor block and native API transport.');
} catch (error) {
