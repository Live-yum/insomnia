...
test('can use client certificate for mTLS', async ({ app, page, insomnia }) => {
  ...
  const certificatesFixturePath = getFixturePath('certificates');
  await page.getByTestId('dataFolders').fill(certificatesFixturePath);
  await page.getByTestId('dataFolders-btn').click();
  await expect.soft(page.getByText(certificatesFixturePath, { exact: true })).toBeVisible({ timeout: UI_TIMEOUT });
