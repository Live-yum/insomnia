...
test('importing external vault references preserves the request without cloud authentication', async ({ app, page, insomnia }) => {
  ...
  await expect(page.getByRole('button', { name: 'Create Cloud Credential', exact: true })).toHaveCount(0);
  await expect(page.getByText('Authenticate With Azure', { exact: true })).toHaveCount(0);
  ...
  expect(importedAfter?.body).toEqual(importedBefore?.body);
  expect(JSON.stringify(importedAfter?.body)).toContain("{% vault 'aws'");
  expect(JSON.stringify(importedAfter?.body)).toContain("{% vault 'gcp'");
  expect(JSON.stringify(importedAfter?.body)).toContain("{% vault 'hashicorp'");
});
