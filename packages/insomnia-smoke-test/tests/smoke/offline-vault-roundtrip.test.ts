...
test('local vault unlocks with its actual key after locking and reloading without a vendor account', async ({ page }) => {
  ...
  await expect(display).not.toHaveText('');
  ...
  await expect(page.getByTestId('VaultKeyDisplayPanel')).toHaveText(key);
  const session = await page.evaluate(() => window._dataServicesInvoke('userSession', 'get'));
  expect(session.id).toBe('');
  expect(session.accountId).toBe('');
  expect(session.vaultKey).not.toBe(key);
  expect(session.offlineVaultProof).toBeTruthy();
});
