...
import { decodeJwt, decryptAesGcm, digest, encryptAesGcm, hmac, offlineToolkit, signJwt, totp } from './offline-toolkit';
...
describe('bundled offline toolkit', () => {
  ...
  it('matches SHA-256 and HMAC vectors', () => {
    ...
    expect(hmac('The quick brown fox jumps over the lazy dog', 'key')).toBe('f7bc83f430538424b13298e6aa6fb143ef4d59a14946175997479dbc2d1a3cd8');
  });
  it('uses fresh GCM IVs and rejects tampered ciphertext and wrong keys', () => {
    ...
    const tampered = JSON.parse(first); tampered.tag = Buffer.alloc(16).toString('base64');
  ...
  it.each([[59, '94287082'], [1111111109, '07081804'], [1111111111, '14050471'], [1234567890, '89005924'], [2000000000, '69279037'], [20000000000, '65353130']])('matches RFC 6238 at %i', (seconds, expected) => {
