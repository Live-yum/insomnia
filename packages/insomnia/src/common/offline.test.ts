...
describe('offline network policy', () => {
  ...
  it.each([
    'https://insomnia-app.local/organization/org_offline/project',
    'file:///app/index.html',
    'http://127.0.0.1:8080',
    'http://10.1.2.3',
    'http://172.16.0.1',
    ...
    'http://[::1]',
    'http://[fd00::1]',
  ])('allows local %s', url => expect(isOfflineNetworkUrlAllowed(url)).toBe(true));
  it.each([
    'https://api.insomnia.rest/v1',
    'https://updates.insomnia.rest',
    'https://api.segment.io',
    'https://o1147619.ingest.sentry.io',
    ...
    'javascript:alert(1)',
    'not a URL',
  ])('blocks %s', url => expect(isOfflineNetworkUrlAllowed(url)).toBe(false));
  ...
  it('never calls the cloud transport, even with injected configuration and credentials', async () => {
    ...
    await expect(
      insomniaFetch({ method: 'GET', path: '/v1/test', sessionId: 'old-cloud-session' }),
