...
func call(t *testing.T, o map[string]any) Result {
	t.Helper()
...
func reject(t *testing.T, o map[string]any) {
	t.Helper()
	raw, _ := json.Marshal(o)
	r, e := execute(raw)
	if e == nil || r != nil {
		t.Fatal("invalid input returned a result")
	}
}
...
func TestPublishedSM2Vectors(t *testing.T) {
	...
	if !call(t, map[string]any{"action": "verify", "algorithm": "SM2", "key": pub, "input": "ShangMi SM2 Sign Standard", "signature": sig, "signatureEncoding": "hex"})["valid"].(bool) {
	...
	if got["output"] != "send reinforcements, we're going to advance" {
		t.Fatal("published ciphertext differs")
	}
}

func TestSM2KeyFormatsAndIntegrity(t *testing.T) {
	for _, format := range []string{"pem", "der", "hex"} {
		t.Run(format, func(t *testing.T) {
			keys := call(t, map[string]any{"action": "keygen", "algorithm": "SM2", "format": format})
			priv := map[string]any{"format": format, "data": keys["privateKey"]}
			pub := map[string]any{"format": format, "data": keys["publicKey"]}
			for _, mode := range []string{"asn1", "c1c3c2", "c1c2c3"} {
				...
				encrypted := call(t, o)["output"].(string)
				if encrypted == call(t, o)["output"] {
				...
				if call(t, d)["output"] != "000180ff" {
					t.Fatal("binary mismatch")
				}
				data, _ := base64.StdEncoding.DecodeString(encrypted)
				data[len(data)-1] ^= 1
				d["input"] = base64.StdEncoding.EncodeToString(data)
				reject(t, d)
			}
...
func TestSM3AndSM4KnownAnswers(t *testing.T) {
	if call(t, map[string]any{"action": "digest", "algorithm": "sm3", "input": "abc"})["output"] != "66c7f0f462eeedd9d1f2d46bdc10e4e24167c4875cf2f7a2297da02b8f4ba8e0" {
		t.Fatal("SM3 vector")
	}
	k := "0123456789abcdeffedcba9876543210"
	if call(t, map[string]any{"action": "cipher", "algorithm": "sm4-cbc", "operation": "encrypt", "key": k, "iv": "00000000000000000000000000000000", "input": k, "inputEncoding": "hex", "outputEncoding": "hex", "padding": "none"})["output"] != "681edf34d206965e86b3e94f536e4246" {
		t.Fatal("SM4 vector")
	}
	for _, mode := range []string{"sm4-cbc", "sm4-ctr", "sm4-gcm"} {
		for _, text := range []string{"", "国密中文 🔒"} {
			e := call(t, map[string]any{"action": "cipher", "algorithm": mode, "operation": "encrypt", "key": k, "input": text})
			d := map[string]any{"action": "cipher", "algorithm": mode, "operation": "decrypt", "key": k, "input": e["output"], "inputEncoding": "base64", "outputEncoding": "utf8", "iv": e["iv"]}
			if mode == "sm4-gcm" {
				d["tag"] = e["tag"]
			}
			if call(t, d)["output"] != text {
				t.Fatal("SM4 roundtrip")
			}
			if mode == "sm4-gcm" {
				d["tag"] = "00000000000000000000000000000000"
				reject(t, d)
			}
		}
	}
...
func TestProtocolRejectsMalformedAndUnknownInputs(t *testing.T) {
	for _, raw := range []string{"{}", "null", "{\"action\":\"digest\",\"algorithm\":\"sm3\",\"path\":\"/secret\"}", "{} {}", "[1]"} {
		if r, e := execute([]byte(raw)); e == nil || r != nil {
			t.Fatal("invalid protocol accepted")
		}
	}
	for _, encoded := range []string{"YQ==\n", "YQ=", "YR==", "%%%%"} {
		reject(t, map[string]any{"action": "digest", "algorithm": "sm3", "input": encoded, "inputEncoding": "base64"})
	}
...
func TestSHA3PublishedAnswer(t *testing.T) {
	if call(t, map[string]any{"action": "digest", "algorithm": "sha3-256", "input": "abc"})["output"] != "3a985da74fe225b2045c172d6bd390bd855f086e3e9d525b46bfe24511431532" {
		t.Fatal("SHA3 vector")
	}
}
...
func TestOpenSSLInteroperability(t *testing.T) {
	...
	openssl, e := exec.LookPath("openssl")
	if e != nil {
		t.Fatal("OpenSSL interop verifier is required")
	}
	dir := t.TempDir()
	keys := call(t, map[string]any{"action": "keygen", "algorithm": "SM2"})
	priv := filepath.Join(dir, "private.pem")
	pub := filepath.Join(dir, "public.pem")
	message := filepath.Join(dir, "input.bin")
	sigfile := filepath.Join(dir, "signature.der")
	cipherfile := filepath.Join(dir, "cipher.der")
	for file, data := range map[string]string{priv: keys["privateKey"].(string), pub: keys["publicKey"].(string), message: "independent 中文\x00\xff"} {
	...
	run := func(args ...string) []byte {
		...
		if err != nil {
			t.Fatalf("OpenSSL failed: %s", out)
		}
		return out
	}
	run("pkeyutl", "-encrypt", "-pubin", "-inkey", pub, "-in", message, "-out", cipherfile)
	ciphertext, e := os.ReadFile(cipherfile)
	if e != nil {
		t.Fatal(e)
	}
	...
	original, e := os.ReadFile(message)
	if e != nil {
		t.Fatal(e)
	}
	if d["output"] != hex.EncodeToString(original) {
		t.Fatal("OpenSSL encryption -> helper decryption mismatch")
	}
	...
	ciphertext, e = hex.DecodeString(enc["output"].(string))
	if e != nil {
		t.Fatal(e)
	}
	if e = os.WriteFile(cipherfile, ciphertext, 0600); e != nil {
		t.Fatal(e)
	}
	if !bytes.Equal(run("pkeyutl", "-decrypt", "-inkey", priv, "-in", cipherfile), original) {
		t.Fatal("helper encryption -> OpenSSL decryption mismatch")
	}
	run("pkeyutl", "-sign", "-rawin", "-digest", "sm3", "-pkeyopt", "distid:offline-interop", "-inkey", priv, "-in", message, "-out", sigfile)
	sig, e := os.ReadFile(sigfile)
	if e != nil {
		t.Fatal(e)
	}
	if !call(t, map[string]any{"action": "verify", "algorithm": "SM2", "key": keys["publicKey"], "input": hex.EncodeToString(original), "inputEncoding": "hex", "signature": hex.EncodeToString(sig), "signatureEncoding": "hex", "userId": "offline-interop"})["valid"].(bool) {
	...
	sig, e = hex.DecodeString(signed["output"].(string))
	if e != nil {
		t.Fatal(e)
	}
	if e = os.WriteFile(sigfile, sig, 0600); e != nil {
		t.Fatal(e)
	}
